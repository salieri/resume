...
## Execution Protocol
Execute all steps. Operate autonomously with the goal of fully resolving the CI failure.
...
4. **Save diagnosis**: Before making any changes, store a clear, concise diagnosis in this format into `{{ARTIFACT_PATH}}/diagnosis.txt`:
   ```
   DIAGNOSIS:
...
1. **Summarize changes** in this format and save the summary in `{{ARTIFACT_PATH}}/resolution.txt`:
   ```
   RESOLUTION SUMMARY:
   - Files modified: <list>
...
1. Document what you tried and why it failed and save the summary in `{{ARTIFACT_PATH}}/failure.txt`:
    ```
    UNABLE TO RESOLVE:
